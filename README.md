...
- **Actions fail on Trivy CRITICAL:** the workflow fails if Trivy finds any **CRITICAL** in the built images. The Dockerfiles run **`apt-get upgrade`** (API/worker) and **`apk upgrade`** (frontend) in the final stage to pick up patched OS libraries. Re-run the workflow after pulling latest base images (`docker compose build --pull`). If it still fails, open the **security** job log and SARIF artifact to see the exact CVE and package.
